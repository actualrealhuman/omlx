    ...
    for i, cache_obj in enumerate(cache_list):
        ...
        for sub in sub_caches:
            ...
            if name in _ARRAYS_SUB_CLASSES:
                ...
                descriptors.append(name or "?")
        if descriptors and has_non_sliceable:
            sub_names = [_canonical_sub_name(type(sub).__name__) for sub in sub_caches]
...
class GDNCheckpointIndex:
    ...
    def remove_key(self, key: tuple[str, bytes]) -> GDNCheckpointMetadata | None:
        with self._lock:
            metadata = self._index.pop(key, None)
            if metadata is None:
                return None
...
class PagedSSDCacheManager(CacheManager):
    ...
    def _record_promotion_failure(self, block_hash: bytes, exc: Exception) -> None:
        ...
        logger.warning(
            "Hot cache promotion failed for block %s: %s (%d promotion failures total)",
            block_hash.hex()[:16],
            exc,
        ...
        *,
        ...
        with self._lock:
            try:
                ...
                if sidecar_root.is_symlink() or sidecar_root.resolve(
                    strict=True
                ).parent != self._cache_dir.resolve(strict=True):
                    logger.warning("Rejecting unsafe GDN sidecar root")
                    return None
                final_path.parent.mkdir(parents=True, exist_ok=True)
                if final_path.parent.is_symlink() or final_path.parent.resolve(
                    strict=True
                ).parent != sidecar_root.resolve(strict=True):
        ...
        with self._lock:
            ...
            for candidate_index, candidate in enumerate(candidates):
                ...
                if candidate_metadata is None:
                    continue
                if not self._is_safe_gdn_sidecar_file(candidate_metadata.file_path):
                    self._gdn_sidecar_index.remove(source_block_hash, candidate_digest)
                    continue
            ...
            return GDNCheckpointLookup(
                ...
                effective_state_codec=_GDN_STATE_CODEC_BY_DTYPE[effective_state_dtype],
        ...
        with self._lock:
            for candidate in self._gdn_signature_candidates(cache_signature):
                ...
                if metadata.file_path.is_file():
                    return True
                self._gdn_sidecar_index.remove(source_block_hash, signature_digest)
            return False
    ...
    def _is_compatible_cache_signature(self, metadata: PagedSSDBlockMetadata) -> bool:
        ...
        if not metadata.cache_signature:
            ...
            return self._payload_layout == "embedded" and self._signature_bits_match("")
    ...
    def signature_mismatch_reason(self, cache_signature: str) -> str | None:
        ...
        if actual_layout != self._payload_layout:
            return (
                f"payload layout: expected {self._payload_layout}, got {actual_layout}"
            )
    ...
    def cache_signature_for(
        ...
        *,
        ...
        return _cache_compat_signature(
            ...
            num_layers=num_layers,
            block_size=block_size,
            layer_cache_types=_canonicalize_layer_cache_types(layer_cache_types) or [],
            turboquant_kv_bits=turboquant_kv_bits,
            cachelist_subtypes=cachelist_subtypes,
    ...
    def _write_inspection(
        ...
        try:
            ...
            self._stats["inspection_errors"] += 1
            logger.warning(
                "Inspection size reconciliation failed (%s)", type(exc).__name__
            )
    ...
    def load_block(self, block_hash: bytes) -> list[Any] | None:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "Failed to remove corrupted SSD cache block files %s: %s",
                    file_path,
                    e,
                )
            return None
    ...
    def _evict_tracked_until_size(
        ...
        while self._tracked_ssd_size() > target_size:
            ...
            if compatible:
                candidates.append(
                    (compatible[0].last_access, 0, self._index, compatible[0])
                )
            if incompatible:
                candidates.append(
                    (
    ...
    def clear_hot_cache(self) -> int:
        ...
        for i, (block_hash, entry) in enumerate(entries):
            if self._writer_thread and not self._writer_thread.is_alive():
                ...
                break
            if entry.get("dirty", True) and self._enqueue_ssd_write(block_hash, entry):
                flushed += 1
        if entries:
            logger.info(
