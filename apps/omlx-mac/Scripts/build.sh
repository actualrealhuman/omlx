...
BUILD_MANIFEST="$REPO_ROOT/omlx/_build_manifest.json"
...
if [ "$WITH_CUSTOM_KERNEL" = "1" ]; then
    BUILD_FEATURES="${BUILD_FEATURES},custom-kernels"
fi
...
log "Writing private build identity..."
cat > "$RESOURCES_DIR/omlx/_build_info.py" <<EOF
# Generated by apps/omlx-mac/Scripts/build.sh. Do not edit.
build_number = "$BUILD_NUMBER"
build_channel = "$BUILD_CHANNEL"
source_revision = "$SOURCE_REVISION"
source_branch = "$SOURCE_BRANCH"
build_features = tuple(filter(None, "$BUILD_FEATURES".split(",")))
EOF
ok "  + _build_info.py ($BUILD_CHANNEL; $BUILD_FEATURES)"
...
# Keep the canonical semantic version untouched for updater compatibility;
# downstream identity lives in separate keys consumed by About/Status.
/usr/libexec/PlistBuddy -c "Delete :OMLXBuildChannel" "$INFO_PLIST" 2>/dev/null || true
/usr/libexec/PlistBuddy -c "Add :OMLXBuildChannel string $BUILD_CHANNEL" "$INFO_PLIST"
/usr/libexec/PlistBuddy -c "Delete :OMLXSourceRevision" "$INFO_PLIST" 2>/dev/null || true
/usr/libexec/PlistBuddy -c "Add :OMLXSourceRevision string $SOURCE_REVISION" "$INFO_PLIST"
/usr/libexec/PlistBuddy -c "Delete :OMLXSourceBranch" "$INFO_PLIST" 2>/dev/null || true
/usr/libexec/PlistBuddy -c "Add :OMLXSourceBranch string $SOURCE_BRANCH" "$INFO_PLIST"
/usr/libexec/PlistBuddy -c "Delete :OMLXBuildFeatures" "$INFO_PLIST" 2>/dev/null || true
/usr/libexec/PlistBuddy -c "Add :OMLXBuildFeatures array" "$INFO_PLIST"
IFS=',' read -r -a BUILD_FEATURE_ARRAY <<< "$BUILD_FEATURES"
for feature in "${BUILD_FEATURE_ARRAY[@]}"; do
    [ -n "$feature" ] || continue
    /usr/libexec/PlistBuddy -c "Add :OMLXBuildFeatures: string $feature" "$INFO_PLIST"
done
