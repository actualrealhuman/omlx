...
- Implementation status: phase 1 is `0001`; phases 2 (per-chat records) and 3
  (IndexedDB backend with atomic revision locking) are implemented on
  `feature/chat-storage-indexeddb`; phase 4a (media preservation —
  content-addressed `blobs`, inline attachments preserved across reload,
  mark-and-sweep collector) is implemented, with upload-time thumbnail
...
The node IndexedDB tests run against `tests/helpers/fake_indexeddb.cjs`, a
...
not a browser — it once modelled `IDBCursor.continue()` as returning a request
when Chrome returns `undefined` — so a green node run is not by itself proof of
browser correctness.

A real-browser harness now lives at `tests/browser/` (`bash tests/browser/run.sh`;
